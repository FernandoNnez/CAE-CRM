document.addEventListener("DOMContentLoaded", function () {
    ...
    // ASP.NET MVC genera automáticamente el id="Password" para asp-for="Password"
    const passwordInput = document.getElementById("Password");
    const eyeIcon = document.getElementById("eyeIcon");

    // Paths SVG para alternar
    ...
    toggleBtn.addEventListener("click", function () {
        ...
        // Cambia el título del botón y el ícono
        if (type === "password") {
            toggleBtn.title = "Mostrar contraseña";
            eyeIcon.innerHTML = `<path d="${pathOjoCerrado}"></path>`;
        } else {
            toggleBtn.title = "Ocultar contraseña";
            eyeIcon.innerHTML = `<path d="${pathOjoAbierto}"></path>`;
        }
    });
});